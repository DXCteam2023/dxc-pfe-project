      ...
      <table className="table-auto w-full border-2">
        ...
        <tbody>
          {characteristics?.map((item: any) => (
            <tr className="border-2 text-center cursor-pointer hover:bg-[#ebebeb]">
              <td className="border-r p-2">{item.name}</td>
              <td className="border-r p-2 flex gap-2 justify-center">
                {item.value ? (
                  item.value
                ) : (
                  <div className="text-red-700">required</div>
                )}
                <AiOutlineEdit onClick={() => handleItemOnClick(item)} />
              </td>
            </tr>
          ))}
        </tbody>
