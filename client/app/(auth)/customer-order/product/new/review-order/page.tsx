...
export default function ReviewOrder() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < myContext?.productOrders.length; i++) {
      ...
      const locationFound = myContext.locations.find(location => location.value === currentProductOrder.locationId)
      ...
      for (let j = 0; j < currentProductOrder?.offerings.length; j++) {
        ...
        tmp.push({
          ...
          productSpecification: currentOffering.productOfferingObject
          ...
          price: ((currentOffering?.productOfferingObject
            ?.productOfferingPrice?.[0]?.price?.taxIncludedAmount
            ?.value || 0) + (currentOffering?.productOfferingObject
              ?.productOfferingPrice?.[1]?.price?.taxIncludedAmount
              ?.value || 0)) * (currentOffering.quantity || 1)
        })
      }
  ...
  return (
    <div className="review-order w-full p-4">
      ...
      <div className="flex gap-2 items-center p-2">
        ...
        <div className="bg-[#5f249f] w-8 h-8 flex justify-center items-center text-white  rounded-[50%] p-2">{orderItems?.length}</div>
      </div>
      <div className="flex gap-4">
        <table className="table-auto w-full border-2">
          ...
          <tbody>
            {orderItems?.map((item, index) => (
              <tr className="border-2 text-center cursor-pointer hover:bg-[#ebebeb]" onClick={() => handleOrderItemOnClick(index)}>
        ...
        <div>
          {selectedOrderItem && <ItemDetails item={selectedOrderItem} onClose={handleSelectedOrderItemOnClose} />}
        </div>
      </div>
      <div className="flex justify-end border-t-2 mt-2 p-2">
        <div className="p-2 rounded-[5px] bg-[#c19de8] text-white">Total price {orderItems?.reduce((total, current) => total + current.price, 0)} USD</div>
      </div>
    </div>
  );
}
