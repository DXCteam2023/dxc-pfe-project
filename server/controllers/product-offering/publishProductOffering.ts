...
export default async function publishProductOffering(req: Request, res: Response) {
  ...
  try {
    ...
    const response = await axios.patch(url, payload, { auth, headers: { 'Content-Type': 'application/json' } });
  
    if (response.status === 200) {
      console.log("Product offering published successfully in ServiceNow.");
    } else {
      console.error("Failed to publish the Product offering in ServiceNow", response.data);
    }
