...
export default async function publishProductOffering(
  ...
  try {
    ...
    const url = `https://dev174830.service-now.com/api/sn_ind_tmt_orm/catalogmanagement/productoffering/${id}`;
    const auth = {
      username: "admin",
    ...
    if (response.status === 200) {
      ...
      console.error(
        "Failed to publishing the Product offering in ServiceNow",
        response.data
      );
    }
